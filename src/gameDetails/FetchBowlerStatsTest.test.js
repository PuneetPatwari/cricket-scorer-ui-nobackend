...
describe('test cases of fetchBowlerStats function', () => {
  it('should return overs count as 0 if the bowler did not bowled any over', () => {
    ...
    const { totalOversBowled } = fetchBowlerStats('Wasim', overDetails);
  ...
  it('should return overs count bowled by the bowler as 1.5', () => {
    const overDetails = [
      {
        bowler: 'Wasim',
        ballDetails: [
    ...
    const { totalOversBowled } = fetchBowlerStats('Wasim', overDetails);
    expect(totalOversBowled).toEqual(1.5);
  });

  it('should return overs count bowled by the bowler as 1.4', () => {
    const overDetails = [
      {
        bowler: 'Wasim',
        ballDetails: [
    ...
    const { totalOversBowled } = fetchBowlerStats('Wasim', overDetails);
    expect(totalOversBowled).toEqual(1.4);
  });
  ...
  it('should return 2 extra runs if no-ball and wide is bowled in that over', () => {
    ...
    const { totalRunsConceded } = fetchBowlerStats('Wasim', overDetails);
    expect(totalRunsConceded).toEqual(2);
  });
});
